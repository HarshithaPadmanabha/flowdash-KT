...
export const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    ...
    <div className="min-h-screen bg-background"> 
      {/* Global theme toggle — fixed, non-intrusive */}
      ...
      <header className="lg:hidden sticky top-0 z-50 bg-card shadow-md p-4 flex items-center justify-between">
        ...
        {/* Mobile Logo (Right) */}
        <div className="h-8 w-8 bg-primary rounded-full flex items-center justify-center text-primary-foreground text-lg font-bold order-2"> 
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-60 bg-[#2a00b7] text-white flex flex-col shadow-lg 
          ...
          lg:translate-x-0 ${isSidebarCollapsed ? "lg:w-16" : "lg:w-60"} lg:h-screen`} // Removed lg:static
      >
        {/* Desktop Logo Header */}
        <div className={`flex items-center h-20 border-b border-white/20 ${isSidebarCollapsed ? "justify-center px-2" : "justify-between px-4"}`}>
          {/* Logo */}
          {!isSidebarCollapsed && (
            <img src={DESKTOP_LOGO_URL} alt="Logo" className="w-40 max-w-full flex-shrink-0" />
          )}
          {isSidebarCollapsed && (
            <div className="h-10 w-10 bg-primary rounded-full flex items-center justify-center text-primary-foreground font-bold flex-shrink-0">
              {MOBILE_LOGO_PLACEHOLDER}
            </div>
          )}
          ...
          {!isSidebarCollapsed && (
            <Button
              ...
              className="hidden lg:flex text-white hover:bg-white/20 transition-all flex-shrink-0"
        ...
        <nav className={`flex-1 overflow-y-auto py-6 space-y-2 ${isSidebarCollapsed ? "px-2" : "px-4"}`}>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center px-4 py-3 rounded-lg font-medium cursor-pointer transition-all duration-200 ${
        ...
        {/* Footer */}
        <div className={`border-t border-white/20 ${isSidebarCollapsed ? "p-2" : "p-4"}`}>
          <div className="bg-white/10 rounded-lg p-3 text-sm mb-3">
            <div className="flex items-center gap-2">
              <Users className="h-4 w-4 text-white/80" />
              {!isSidebarCollapsed && <span className="capitalize">{role}</span>}
            </div>
            {!isSidebarCollapsed && (
          ...
          <Button
            ...
            className={`w-full flex items-center text-white hover:bg-red-600/80 transition-colors ${
      ...
      <main className={`${isSidebarCollapsed ? "lg:ml-16" : "lg:ml-60"} flex-1 overflow-y-auto min-h-screen`}>
        <div
          className={
